...
class TransportationProblem(Problem):
    ...
    def actions(self, state):
        valid_actions = []
        if state + 1 <= self.n: # comprobamos que no nos salimos
            valid_actions.append("walk")
        if 2 * state <= self.n: # comprobamos que no nos salimos
    ...
    def path_cost(self, c, state1, action, state2): # c es el coste hasta el estado 1
        if action == "walk":
            return c + 1
        else:
            return c + 2
